...
//            return ResponseEntity.noContent().build();
    ...
    public ResponseEntity<Manager> mannupdate(@RequestBody Manager mann,@PathVariable Long id){
        Optional<Manager> mannn=repo2.findById(id);
        if(mannn.isPresent()){
            Manager man1=null;
            Optional<Manager> mnn=repo2.findById(id);
            man1=mnn.get();
            man1.setId(id);
            man1.setName(mann.getName());
            man1.setDept(mann.getDept());
            return ResponseEntity.ok().body(repo2.save(man1));
        }
        else{
            return ResponseEntity.noContent().build();
        }
